...
const LanguageSwitcher = () => {
  ...
  return (
    <button
      ...
      className="relative p-2 rounded-full bg-transparent hover:bg-muted transition-all duration-300 group"
      aria-label="Toggle language"
    >
      <div className="relative w-5 h-5 flex items-center justify-center overflow-hidden">
        <span
          className={`absolute text-xs font-bold transition-all duration-300 transform ${
            language === 'id' ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-4'
          }`}
        >
          ID
        </span>
